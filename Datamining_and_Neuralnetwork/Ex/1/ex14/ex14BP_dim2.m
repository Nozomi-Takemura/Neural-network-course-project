...
for m = [30,150,450,1200]
    ...
    for i = 1:m
        ...
        %y(1,i) = sinc(sqrt(norm(inp_tyh(:,1),2)))
    end
    ...
    for k = [1,3,5,7]%k=3
        ...
        title(['Approximated function against sinc(r): resilient Backpropagation with the number of traing data=' num2str(m) ', amount of noise = 0.1, number of hidden units=' num2str(k) ', dimension = 2']);
        saveas(h,sprintf('BP_data%i_noise01_huni_%i_yhat_vs_sinc.eps',m, k),'epsc'); % will create FIG1, FIG2,...
        %hold off;
        u = count_k;
        matrix_sum(u,1) = m;
        %matrix(u,2) = j;
...
t = toc;
save('C:\Temp\fig14\BP_dim2\stat.mat', 'matrix_sum')
save('C:\Temp\fig14\BP_dim2\stat.mat', 't')
save('C:\Temp\fig14\BP_dim2\stat.mat', 'seq')
save('C:\Temp\fig14\BP_dim2\all.mat')