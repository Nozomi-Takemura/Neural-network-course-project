...
cd /home/nozomi/Documents/2017courses/DMNN/Ex/1/fig14/2d/QN2d
%sec = cell(5,1);
 %m = 10; k = 1
%tic;
...
%Data Generation
    ...
    for k = [5,20,60,100]%k=
        ...
        hold off;
        saveas(g,sprintf('QN_data%i_noise02_huni_%i_reg000001_yhat_vs_sinc_2d.png',m, k)); % will create FIG1, FIG2,...
        %close;
        ...
        save(sprintf('QN_ndata%i_nneurons%i_2d_summary.mat',m, k), 'matrix_sum')
        %save(sprintf('QN_ndata%i_nneurons%i_2d_time.mat',m, k), 't')
        save(sprintf('QN_ndata%i_nneurons%i_2d_storule.mat',m, k), 'seq')
    end
    %end
