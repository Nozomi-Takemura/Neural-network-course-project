...
cd /home/nozomi/Documents/2017courses/DMNN/Ex/1/fig14/1d/BP1d
%sec = cell(5,1);
 %m = 150; k = 5
%tic;
...
%Data Generation
    ...
    for k = [5,20,60,100]%k=
        ...
        test_output=sinc(sqrt((test_intermed)));
        % 
        ...
        plot(test_input,test_output,'b*');
        %tit1=title('Plot of $sinc(\sqrt{x_{1}^{2}} \text{for} −5 ≤ x_{1}, x_{2} ≤ 5$.')
        %set(tit1,'Interpreter','latex');
        xlabel('Input - 1stt dimension')
        ylabel('Output')
        %set(z1,'Interpreter','latex');
        hold on;
        plot(test_input,test_yhat,'ro');
        leg1 = legend({'$sinc(\sqrt{x_{1}^{2}})$','Approximated function'}, 'Location', 'Best');
        set(leg1,'Interpreter','latex');
        hold off;
        saveas(g,sprintf('BP_data%i_noise02_huni_%i_reg000001_yhat_vs_sinc_1d.png',m, k)); % will create FIG1, FIG2,...
        %close;
        ...
        save(sprintf('BP_ndata%i_nneurons%i_1d_summary.mat',m, k), 'matrix_sum')
        %save(sprintf('QN_ndata%i_nneurons%i_2d_time.mat',m, k), 't')
        save(sprintf('BP_ndata%i_nneurons%i_1d_storule.mat',m, k), 'seq')
    end
    %end
